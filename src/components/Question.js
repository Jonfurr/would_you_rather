    ...
    return (
      ...
      <div className='question'>
        ...
        <div className='answers'>
          ...
          <button className={selectedTwo} disabled={disabled} onClick={this.handleAnswerQuestion} value='optionTwo'>{question.optionTwo.text}</button>
        </div>
        <div className={
          answer
          ? 'answered-stats'
          ...
          <div className='option-one-stats center'>
            <p>{question.optionOne.votes.length} people chose {question.optionOne.text}</p>
          ...
          <div className='option-one-stats center'>
            <p>{question.optionTwo.votes.length} people chose {question.optionTwo.text}</p>
            <span className="percent">{
              
              (question.optionTwo.votes.length / (question.optionOne.votes.length + question.optionTwo.votes.length) * 100).toFixed(2)}%</span>
          </div>
        </div>
        <div className='question-data center'>
          <img alt={`${users[question.author]}'s avatar`}className='avatar' src={users[question.author].avatarURL} />
