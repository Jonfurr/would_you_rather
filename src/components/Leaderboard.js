  ...
  render() {
    ...
    return (
      <div className='user-card' style={inlineStyles}>
        <img className='avatar' alt={`${user.name}'s avatar'`} src={user.avatarURL}/>
        <h3>{user.name}</h3>
        <div className='leaderboard-stats'>
          <span>{`Questions asked: ${user.questions.length}`}</span>
          <span>{`Questions answered: ${Object
              .keys(user.answers)
              .length}`}</span>
        </div>
      </div>
    ...
    return (
      <div className='container'>
        ...
        <div className='leaderboard'>
          {userIds.map((id) => (<UserCard key={id} users={users} id={id} score={sorted[id]}/>))}
        </div>
      </div>
    )
  }
